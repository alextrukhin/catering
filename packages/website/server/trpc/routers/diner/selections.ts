...
import { dinerProcedure, router } from "../../trpc";
...
myDay: dinerProcedure
.input(z.object({ id: idSchema }))
.query(async ({ input, ctx }) => {
const record = await readClient.planDayDiner.findFirst({
where: { id: input.id, diner_id: ctx.diner!.id },
select: {
id: true,
PlanDay: {
select: {
...
select: {
id: true,
Meal: { select: { id: true, label_uk: true, label_en: true } },
Courses: {
select: {
id: true,
Course: {
...
where: { id: input.plan_day_diner_id, diner_id: ctx.diner!.id },
select: { id: true },
});
...
where: { id: input.source_id, diner_id: ctx.diner!.id },
select: {
DishSelections: {
...
where: { id: { in: input.target_ids }, diner_id: ctx.diner!.id },
select: { id: true },
});
