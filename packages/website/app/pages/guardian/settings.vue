...
const { data: sessions, refresh: refreshSessions } =
	await $client.guardian.user.listSessions.useQuery(undefined, { lazy: true });

async function removeSession(id: number) {
	try {
		await $client.guardian.user.deleteSession.mutate({ id });
...
const { data: passkeys, refresh: refreshPasskeys } =
	await $client.guardian.user.listPasskeys.useQuery(undefined, { lazy: true });

async function removePasskey(id: number) {
	try {
		await $client.guardian.user.deletePasskey.mutate({ id });
...
const { data: tgStatus, refresh: refreshTgStatus } =
	await $client.guardian.user.telegramStatus.useQuery(undefined, {
		lazy: true,
	});
...
async function linkTelegram() {
	...
	tgBusy.value = true;
	try {
		await $client.guardian.user.linkTelegram.mutate({
...
async function unlinkTelegram() {
	tgBusy.value = true;
	try {
		await $client.guardian.user.unlinkTelegram.mutate();
				...
				<ul v-else class="divide-y divide-default">
					...
					>
						<div>
							...
							<p class="text-xs text-muted">{{ formatDate(pk.created_at) }}</p>
						</div>
						<UButton
							size="xs"
							color="error"
