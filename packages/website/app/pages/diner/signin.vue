...
onMounted(async () => {
	if (!webApp.initData || !orgId.value || $diner.isAuthorized.value) return;
	try {
		await $client.diner.user.loginWithTelegram.mutate({
			init_data_raw: webApp.initData,
			organization_id: orgId.value,
		});
		// cookies are set; reload to trigger isAuthorized watcher
		await $diner.refresh?.();
		router.replace("/diner/dashboard");
	} catch {
		// No linked account → show normal login (possibly link after)
	}
});
...
const otpChallengeId = useCookie("diner_otp_challenge", {
	maxAge: 5 * 60,
	default: () => 0,
});
...
const sendOtp = async () => {
	...
	try {
		const result = await $client.diner.user.requestOtp.mutate({
			phone: phone.value,
		});
...
const confirmOtp = async () => {
	...
	try {
		const result = await $client.diner.user.verifyOtp.mutate({
...
const loginWithPasskey = async () => {
	...
	try {
		const { options, challenge_id } =
			await $client.diner.user.startPasskeyAuth.mutate();
		const response = await startAuthentication({ optionsJSON: options });
		await $client.diner.user.verifyPasskeyAuth.mutate({
			challenge_id,
			response,
		});
		router.replace("/diner/dashboard");
