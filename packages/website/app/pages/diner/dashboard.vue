...
const {
	...
	pending: calPending,
} = $client.diner.selections.myCalendar.useQuery(
...
const {
	...
	refresh: refreshDay,
	pending: dayPending,
} = $client.diner.selections.myDay.useQuery(
	computed(() => ({ id: expandedId.value ?? 0 })),
...
function onDayClick(day: DayCell) {
	const planDay = planDayMap.value.get(day.key);
	if (planDay) toggleExpand(planDay.id);
}
...
async function onCourseOptionSelect(
	...
	if (busy.value || !dayData.value) return;
	busy.value = true;
	try {
		await $client.diner.selections.setDish.mutate({
...
async function addPasskey() {
	...
	try {
		const { options, challenge_id } =
			await $client.diner.user.startPasskeyRegistration.mutate();
		const response = await startRegistration({ optionsJSON: options });
		await $client.diner.user.verifyPasskeyRegistration.mutate({
			challenge_id,
...
async function executeCopy() {
	if (!copySourceId.value || copyTargetIds.value.length === 0) return;
	copyBusy.value = true;
	try {
		const result = await $client.diner.selections.copyDay.mutate({
		...
		if (expandedId.value) await refreshDay();
	} catch {
	...
	<div class="max-w-3xl mx-auto px-4 py-8 space-y-8">
		...
		<UCard :ui="{ body: 'p-4 sm:p-6' }">
						...
						<div
							...
							:class="
								planDayMap.get(day.key)!.is_complete
									? 'bg-green-500'
									: planDayMap.get(day.key)!.selected_courses > 0
										? 'bg-yellow-400'
										: 'bg-red-400'
							"
						/>
			...
			>
				...
				<div class="flex items-center gap-1.5">
					<span class="size-2 rounded-full bg-yellow-400 inline-block" />
					{{ $t("dashboard.partial") }}
				</div>
				<div class="flex items-center gap-1.5">
		...
		<div v-if="(calendarDays?.length ?? 0) > 0" class="space-y-2">
			...
			<div class="space-y-1">
				...
				>
					<button
						class="w-full flex items-center justify-between px-4 py-3 text-sm bg-background hover:bg-elevated/50 transition-colors text-left"
					...
					<div v-if="expandedId === d.id" class="border-t border-default">
						<div
							...
							:class="{ 'opacity-50 pointer-events-none': dayPending || busy }"
						>
		...
		<p v-else class="text-sm text-center text-muted py-6">
			{{ $t("dashboard.no_days") }}
		</p>
	</div>
