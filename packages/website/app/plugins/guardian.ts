...
export default defineNuxtPlugin((nuxtApp) => {
	...
	const isAuthorizedCookie = useCookie("guardian_authenticated") as Ref<
		string | boolean | null
	>;
	...
	const { data: user, refresh } = $client.guardian.user.me.useQuery(undefined, {
		immediate: false,
	});
	...
	const logout = async () => {
		try {
			await $client.guardian.user.logout.mutate();
			await refresh();
		} catch (e) {
			console.error(e);
		}
