...
export default defineNuxtPlugin((nuxtApp) => {
	...
	const isAuthorizedCookie = useCookie("diner_authenticated") as Ref<
		string | boolean | null
	>;
	...
	const { data: user, refresh } = $client.diner.user.me.useQuery(undefined, {
		immediate: false,
	});
	...
	const logout = async () => {
		try {
			await $client.diner.user.logout.mutate();
			await refresh();
		} catch (e) {
			console.error(e);
		}
